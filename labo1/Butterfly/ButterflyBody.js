class ButterflyBody {
    constructor(parent) {
        ...
        this.colors = this.colors.concat(this.parent.color1); //concat merge the two array (push the values)
        this.indices.push(0);
        ...
        for (let theta = 0; theta < 2 * Math.PI; theta += step) {
            ...
            this.colors = this.colors.concat(this.parent.color2);
            this.indices.push(this.indices.length - 1);
            this.indices.push(0);
        }

        vec3.forEach(this.vertices, 0, 0, 0, vec3.scale, this.parent.scale);
